...
### Google検索グラウンディング（本文生成、2026-09-08導入）

本文執筆（`jaBodyPrompt()` → `callText(ai, prompt, { grounding: true })`）でのみ `tools: [{ googleSearch: {} }]` を
有効化している（メタデータ生成の `jaMetaPrompt()`・翻訳の `transBodyPrompt()` は対象外）。国交省APIの実取引データ
だけではカバーできない「対象エリアの直近1〜2年の再開発計画・新設商業施設/公共インフラ・自治体の最新の住宅補助金や
子育て支援制度」を検索させ、具体的な施設名・計画名・制度名を本文に引用させる（`jaBodyPrompt()` 内の
`groundingBlock`）。ツール利用時にエラーが発生した場合は同じプロンプトを検索なしで再試行し、記事生成を止めない
（[[ai_proposal_google_search_grounding]] の `generateCustomerProposal()` と同じ安全設計）。
`BLOG_DRY_RUN=1` の実行で検証済み: 「軽井沢 T-SITE」（2026年3月開業）、西武不動産による軽井沢プリンスホテルの
約290億円リニューアル、軽井沢町とソフトバンクの自治体DX連携協定（2025年4月）など、検索なしでは出てこない具体的な
固有名詞・時期が本文に反映された。
...
| **[LEGAL-1] 評価・推奨・勧誘語の全面排除** | 出力・UI・プロンプト全箇所から鑑定士ペルソナ・査定語・推奨語を削除。置換方針：「公的データ集計・整理」型。全4言語対応（2026-07） |
| **[LEGAL-1] 利用規約暫定改定（弁護士レビュー前）** | 免責条項→月額上限・重過失除外カービアウト、料金変更30日前通知義務、民法548条の4 参照、Lemon Squeezy 解約手順明記 |
| **[LEGAL-1] 同意フロー追加** | 認証モーダル（利規・PP リンク付き告知）、Proアップグレードに同意チェックボックス追加 |
| **[LEGAL-1] 決済事業者表記統一** | Stripe → Lemon Squeezy を全4言語・全ページで統一（利規/PP/About 含む） |
