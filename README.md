...
7. [テストと品質保証 (QA)](#-テストと品質保証-qa)
8. [ローカル開発環境のセットアップ](#-ローカル開発環境のセットアップ)
9. [Cloud Run へのデプロイ](#-cloud-run-へのデプロイ)
10. [Stripe 決済の本番稼働手順](#-stripe-決済の本番稼働手順)
...
| 8 | 人口の増減予想 | Pro |
| 9 | リアルな住環境と注意点 | Pro |
| 10 | 不動産プロの視点：物件の魅力とおすすめポイント | Pro |

### Step 4 — 暮らしのイメージ画像を生成（Pro プラン）
Google アカウントでログインし Pro プランに加入後、AI レポート内の **「✨ 暮らしイメージを生成」** ボタンをクリック。
...
利用制限の判定は localStorage（ゲスト）と Firestore（Free/Pro）で管理し、
サーバー側で Firebase ID Token を検証してプラン昇格の不正操作を防止しています。
...
[ブラウザ]
  ...
  │  調査フォーム（住所 or 緯度経度入力）
  ▼
[Next.js 15 Frontend / Cloud Run — asia-northeast1]
  │
...
| 技術 | バージョン | 用途 |
|---|---|---|
| Next.js (App Router) | 15 | フレームワーク・SSR/CSR |
...
| @hono/node-server/serve-static | 静的ファイル配信 |
| hono-rate-limiter | IP ベースのレートリミット |
...
| 対策 | 実装内容 |
|---|---|
| **Stripe Checkout の認証強化** | `uid` をリクエストボディで受け付ける脆弱性を修正。Firebase ID Token をヘッダー（`Authorization: Bearer <token>`）で受け取り、サーバー側で `admin.auth().verifyIdToken()` を使って検証。検証済み UID のみ Checkout Session を作成可能 |
...
| **エラーハンドリング明確化** | ハザード・環境情報の取得失敗時は空データ（`EMPTY_HAZARD` / `EMPTY_ENVIRONMENT`）を返却。Gemini 生成失敗時は `undefined` を返却し、フロントエンドで適切に表示 |
...
**全14シナリオ PASS** を確認済み（検証日: 2026-03-30）
...
gcloud run deploy realestate-api \
  ...
  --set-env-vars "ALLOWED_ORIGINS=https://mekiki-research.com"
```
...
├── backend/
│   └── src/
│       ├── index.ts                  # Hono サーバーエントリーポイント・CORS・レートリミット
│       ├── config.ts                 # 環境変数設定
│       ├── routes/
...
│   │   ├── about/                    # サービス紹介・料金プランページ
│   │   ├── terms/                    # 利用規約
│   │   ├── privacy/                  # プライバシーポリシー
│   │   ├── licenses/                 # OSS ライセンス一覧
│   │   ├── globals.css               # グローバルスタイル・PDF制御
│   │   └── layout.tsx
│   ├── components/
│   │   ├── AiReport.tsx              # AI レポートアコーディオン + 画像生成 UI + ペイウォール
...
│   │   ├── PlanComparisonModal.tsx   # 料金プラン比較 + Stripe Checkout 導線
...
│   └── tests/
│       └── production_e2e.spec.ts    # Playwright E2E テスト（本番ドメイン対象）
├── terraform/                        # GCP インフラ IaC（Cloud Run / GCS / Artifact Registry）
├── scripts/
│   └── deploy_frontend.sh            # フロントエンドビルド + Cloud Run デプロイ
├── marketing/
│   ├── launch_tweets.md              # X（Twitter）ローンチ告知ツイートツリー
│   ├── note_story.md                 # note.com 開発ストーリー記事ドラフト
