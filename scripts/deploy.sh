...
gcloud run deploy "$CLOUD_RUN_SERVICE_NAME" \
  ...
  --set-env-vars "^|^GCP_PROJECT_ID=${GCP_PROJECT_ID}|GCP_REGION=${GCP_REGION}|GCS_CACHE_BUCKET=${GCS_CACHE_BUCKET}|BQ_DATASET=${BQ_DATASET:-realestate_cache}|CACHE_TTL_DAYS=${CACHE_TTL_DAYS:-30}|MLIT_API_KEY=${MLIT_API_KEY:-}|GEMINI_API_KEY=${GEMINI_API_KEY:-}|ALLOWED_ORIGINS=${ALLOWED_ORIGINS:-}|ADMIN_EMAILS=${ADMIN_EMAILS:-}|FIREBASE_PROJECT_ID=${FIREBASE_PROJECT_ID:-}" \
  --quiet
