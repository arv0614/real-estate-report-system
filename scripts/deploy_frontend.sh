...
cd "$ROOT_DIR"
SITE_URL="${NEXT_PUBLIC_SITE_URL:-https://realestate-frontend-2hctlfcy6a-an.a.run.app}"
POSTHOG_KEY="${NEXT_PUBLIC_POSTHOG_KEY:-}"
POSTHOG_HOST="${NEXT_PUBLIC_POSTHOG_HOST:-https://us.i.posthog.com}"
GTM_ID="${NEXT_PUBLIC_GTM_ID:-GTM-5ZNNGVZQ}"
GA_MEASUREMENT_ID="${NEXT_PUBLIC_GA_MEASUREMENT_ID:-G-4Y1CLF7J2P}"
gcloud builds submit "$FRONTEND_DIR" \
  --config "$FRONTEND_DIR/cloudbuild.yaml" \
  --substitutions "_NEXT_PUBLIC_API_URL=$BACKEND_URL,_IMAGE=$IMAGE,_NEXT_PUBLIC_SITE_URL=$SITE_URL,_NEXT_PUBLIC_FIREBASE_API_KEY=${NEXT_PUBLIC_FIREBASE_API_KEY},_NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN=${NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN},_NEXT_PUBLIC_FIREBASE_PROJECT_ID=${NEXT_PUBLIC_FIREBASE_PROJECT_ID},_NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET=${NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET},_NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID=${NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID},_NEXT_PUBLIC_FIREBASE_APP_ID=${NEXT_PUBLIC_FIREBASE_APP_ID},_NEXT_PUBLIC_POSTHOG_KEY=$POSTHOG_KEY,_NEXT_PUBLIC_POSTHOG_HOST=$POSTHOG_HOST,_NEXT_PUBLIC_GTM_ID=$GTM_ID,_NEXT_PUBLIC_GA_MEASUREMENT_ID=$GA_MEASUREMENT_ID" \
