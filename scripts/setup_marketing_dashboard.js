 ...
 * Markdown で生成する。GA4 をデータソースに接続する構成、広告効果計測に必要な
 * ディメンション / 指標 / 計算フィールド (インプレッション・CTR・CVR) の定義案を出力する。
 *
 * 使い方:
 *   node scripts/setup_marketing_dashboard.js            # docs/marketing_dashboard.md に書き出す
 *   node scripts/setup_marketing_dashboard.js --stdout   # 標準出力に出すだけ (ファイルは書かない)
 *   node scripts/setup_marketing_dashboard.js --out path  # 出力先を指定
 *
...
try {
  ...
  console.log(`[SUCCESS] ダッシュボード手順書を生成しました: ${path.relative(process.cwd(), OUT_PATH)}`);
