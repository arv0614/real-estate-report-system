...
export function WaitlistModal({ open, onClose, plan, uid, userEmail }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "登録に失敗しました。しばらく後にお試しください。");
    } finally {
      setSubmitting(false);
    }
  }
      ...
      >
        ...
        <div className="bg-gradient-to-br from-slate-800 to-slate-700 px-6 pt-8 pb-6 text-white">
          ...
          <p className="text-sm text-slate-300 leading-relaxed">
            Proプランでは無制限調査・PDF出力・全10項目レポートが使えます。
            先行アクセスのご案内をご希望の方はメールアドレスをご登録ください。
          </p>
        </div>
        ...
        <div className="px-6 py-6">
          {success ? (
            ...
            <div className="text-center py-4">
              ...
              <p className="font-bold text-slate-800 text-base mb-1">ご登録ありがとうございます</p>
              <p className="text-sm text-slate-500 leading-relaxed">
                準備が整い次第、優先してご案内いたします。<br />今しばらくお待ちください。
              </p>
              <button
                onClick={onClose}
            ...
            <>
              <p className="text-sm text-slate-600 mb-4 leading-relaxed">
                公開時にメールでお知らせします。登録は無料・スパムは送りません。
              </p>
