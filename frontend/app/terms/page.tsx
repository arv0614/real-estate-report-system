      ...
      <main className="max-w-3xl mx-auto px-4 py-12">
        ...
        <div className="space-y-10 text-sm leading-relaxed text-slate-700">
          ...
          <section>
            ...
            <ol className="space-y-2 list-none">
              {[
                ...
                "有料プランの決済は、Lemon Squeezyの決済サービスを通じて行われます。クレジットカード情報はLemon Squeezyが直接管理し、運営者のサーバーには保存されません。",
                "Proプランはいつでもキャンセル可能です。解約した場合、当月末日までサービスをご利用いただけます。日割り返金は行っておりません。ただし、サービスの重大な瑕疵により利用できなかった場合は、個別にご相談ください。",
                "料金を変更する場合は、変更の30日前までに本サービス上またはメールにてお知らせします。変更に同意しない場合は、変更の効力が生じる前にプランをキャンセルすることができます。",
                "有料プランのキャンセルは、Lemon Squeezyのカスタマーポータルから行えます。マイページ内の「サブスクリプション管理」から手続きしてください。キャンセル後も、当該請求期間の終了まではProプランをご利用いただけます。",
              ].map((text, i) => (
                <li key={i} className="flex gap-2">
                  <span className="shrink-0 font-semibold text-slate-400">{i + 1}.</span>
                  <span>{text}</span>
          ...
          <section>
            ...
            <ol className="space-y-2 list-none">
              {[
                "本サービスで提供する情報（取引価格データ、エリア特性レポート等）はすべて参考情報です。正確性・完全性・最新性を保証するものではなく、情報の利用によって生じた損害に関する運営者の賠償責任は、原因を問わず、当該損害発生月の月額利用料金を上限とします。ただし、運営者の故意または重大な過失による損害についてはこの限りではありません。",
                "本サービスのシステム障害、メンテナンス、第三者サービス（Firebase、Lemon Squeezy、国土交通省API等）の障害等によりサービスが利用できない場合でも、運営者はそれによって生じた損害について責任を負いません。",
          ...
          <section>
            ...
            <p>
              運営者は、民法第548条の4の規定に基づき、ユーザーの個別の同意を得ることなく本規約を変更することができます。変更を行う場合は、変更の効力が生じる相当期間前に、本サービス上または電子メールにてその旨および変更後の規約内容を周知します。
            </p>
          </section>
